...
export OLD_THREAD_PATH="${NZEDB_ROOT}/misc/update/python"
...
#	if ! $SCREEN -list | grep -q "PTOXY"; then
#		$SCREEN -dmUS PTOXY python ${THREAD_PATH}/nntpproxy.py ${THREAD_PATH}/lib/nntpproxy.conf
#	fi

	cd ${NZEDB_PATH}
	if ! $SCREEN -list | grep -q "POSTP"; then
		cd $NZEDB_PATH && $SCREEN -dmUS POSTP $PHP $NZEDB_PATH/postprocess.php allinf true
	fi
...
#	$PHP ${THREAD_PATH}/backfill.php
	...
	cd ${OLD_THREAD_PATH}
		echo "Start old binaries_threaded.py..."
	$PYTHON -OOu ${OLD_THREAD_PATH}/binaries_threaded.py
		echo "Start old backfill_threaded.py all ..."
	$PYTHON -OOu ${OLD_THREAD_PATH}/backfill_threaded.py all

	cd ${HELP_PATH}
	if ! $SCREEN -list | grep -q "RELEASES"; then
		cd $HELP_PATH && $SCREEN -dmUS RELEASES sh $HELP_PATH/my_helper.sh
	fi
...
#		if ! $SCREEN -list | grep -q "ANIDB"; then
#		echo "Check start populate anidb..."
#			cd $NZEDB_PATH && $SCREEN -dmS ANIDB $PHP ${TEST_PATH}/DB/populate_anidb.php 15 true
#		fi
	fi

	cd ${NZEDB_PATH}
	DIFF=$(($CURRTIME-$LASTOPTIMIZE1))
	if [ "$DIFF" -gt 72000 ] || [ "$DIFF" -lt 1 ]
	then
		LASTOPTIMIZE1=`date +%s`
		echo "Optimizing DB..."
